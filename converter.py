...
class Map:
  ...
  def __init__(self, name, language, groupBy):
    self.paths = {}
    self.name = name
    self.language = language
    self.codesByGroup = False
    if groupBy:
      self.codesByGroup = {}
      for groupCode in groupBy:
        for code in groupBy.get(groupCode):
          self.codesByGroup[code] = groupCode

  def addPath(self, path, code, name):
    if self.codesByGroup:
      if code in self.codesByGroup:
        code = self.codesByGroup[code]
        if not code in self.paths:
          self.paths[code] = {"path": path, "name": code}
        else:
          self.paths[code]['path'] += path
      else:
        self.paths[code] = {"path": path, "name": code}
    else:
      self.paths[code] = {"path": path, "name": code}
...
class Converter:
  def __init__(self, inputFile, **kwargs):
    if kwargs['groupby']:
      groupby = anyjson.deserialize(kwargs['groupby'])
    else:
      groupby = False

    self.map = Map(kwargs['name'], kwargs['language'], groupby)
